...
-- load data
----------------------------------------------------------------
train_seqs,train_scores,valid_seqs,valid_scores = load_data(opt.data_file)
...
if opt.spearmint == '' then
	job.conv_filters = {100,50,50}
	job.conv_filter_sizes = {13,5,3}
	job.pool_width = {6,4,2}
	job.conv_dropouts = {0,0}
	
	job.hidden_units = {600,400}
	job.hidden_dropouts = {0,0}
	job.weight_norm = 5
	job.batch_normalize = true
	job.learning_rate = 0.005
else
	...
	-- scientist = whetlab(opt.whetlab_name, "", nil, nil, true, access_token)
...
if opt.restart ~= '' then
	...
	if build_success == false then
		...
		-- 	scientist:update(job, -math.huge)
		-- end
...
-- update whetlab
-- if scientist ~= nil then
-- 	scientist:update(job, -valid_best)
-- end
